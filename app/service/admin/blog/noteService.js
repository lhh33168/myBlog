...
class noteService extends Controller {
     ...
     */
    get dao() {
        return this.ctx.service.admin.blog.dao.noteDao;
    }
    /**
    * 保存
    * @param {json} body 
    */
    ...
    /**
     * 获取版本发布列表，自动转换blog类型为String类型
     * @param {*} body 
     */
    async list(body) {
        let result = await this.dao.select(body);
        if (result && result.length > 0) {
            for (var i = 0, len = result.length; i < len; i++) {
                result[i].content = result[i].content.toString('utf8', 0);
            }
        }
        return this.st.success('', result);
    }
